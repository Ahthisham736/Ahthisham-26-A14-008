...
# SET YOUR PATHS HERE
# ============================================================
IMAGE_PATH = r"C:\Users\ahthisham1\Desktop\AHTHISHAM 26-A14-008\input\IMG-20260831-WA0025.jpg"
OUTPUT_DIR = r"C:\Users\ahthisham1\Desktop\AHTHISHAM 26-A14-008\outputs"
# ============================================================
...
def process_image(img_path, output_dir):
    ...
    grid_obstacle, grid_level = downsample_grid(obstacle_mask, level_mask, GRID_SCALE)
    ordered_results, full_path = plan_route(
        grid_obstacle, grid_level, GRID_SCALE, start_pos, destination_pos, casualties)

    total_time, per_level_time = compute_total_time(full_path, level_mask)
    total_score = round(sum(c["casualty_score"] for c in ordered_results), 4)

    path_img_out = os.path.join(output_dir, f"{name}_path.png")
    save_path_visualisation(img_bgr, start_pos, destination_pos, ordered_results, full_path, path_img_out)
    ...
    report = {
        ...
        "all_casualties_detected": [
            {"coords": c["coords"], "shape": c["shape"], "age_group": c["age_group"],
             "color": c["color"], "severity": c["severity"], "priority_score": c["priority_score"],
             "level": c["level"]} for c in casualties
        ],
        "visit_order": [
            {"coords": c["coords"], "age_group": c["age_group"], "severity": c["severity"],
             "priority_score": c["priority_score"], "displacement_from_start": c["displacement_from_start"],
             "distance_travelled_to_reach": c["distance_travelled_to_reach"],
             "casualty_score": c["casualty_score"]} for c in ordered_results
        ],
        "casualty_scores": [c["casualty_score"] for c in ordered_results],
        "total_path_score": total_score,
        "path": full_path,
        "total_time_seconds": round(total_time, 3),
        "time_breakdown_by_level_seconds": {str(k): round(v, 3) for k, v in per_level_time.items()},
        "outputs": {"mask_image": mask_out, "path_image": path_img_out},
    }
...
report = process_image(IMAGE_PATH, OUTPUT_DIR)

if "error" in report:
    print(f"[WARN] {report['image']}: {report['error']}")
else:
    print("\n=== DONE ===")
    print("Start position:", report["start_position"])
    print("Destination position:", report["destination_position"])
    print("Number of casualties:", report["num_casualties"])
    print("Casualty scores:", report["casualty_scores"])
    print("Total path score:", report["total_path_score"])
    print("Total time (s):", report["total_time_seconds"])
    print("\nMask image saved to:", report["outputs"]["mask_image"])
    print("Path image saved to:", report["outputs"]["path_image"])
    print("Full report saved to:", os.path.join(OUTPUT_DIR, f"{report['image']}_report.json"))